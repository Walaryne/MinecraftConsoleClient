...
namespace MinecraftClient.Protocol.Session
{
    ...
    public static class SessionCache
    {
        ...
        public static void Store(string login, SessionToken session)
        {
            ...
            if (Settings.SessionCaching == CacheType.Disk && updatetimer.Enabled)
            {
        ...
        {
            cachemonitor = new FileMonitor(AppDomain.CurrentDomain.BaseDirectory, SessionCacheFilePlaintext, OnChanged);
        ...
        private static void HandlePending(object sender, ElapsedEventArgs e)
        {
            ...
            foreach(var pending in pendingadds.ToArray())
            {
        ...
        private static bool LoadFromDisk()
        {
            ...
            if (File.Exists(SessionCacheFileMinecraft))
            {
                ...
                var mcSession = new Json.JSONData(Json.JSONData.DataType.String);
                try
                {
                    ...
                    && mcSession.Properties.ContainsKey("authenticationDatabase"))
                {
                    ...
                    var sessionItems = mcSession.Properties["authenticationDatabase"].Properties;
                    foreach (string key in sessionItems.Keys)
                    {
                        if (Guid.TryParseExact(key, "N", out temp))
                        {
                            var sessionItem = sessionItems[key].Properties;
            ...
            if (File.Exists(SessionCacheFileSerialized))
            {
                ...
                try
                {
                    using (var fs = new FileStream(SessionCacheFileSerialized, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
                        var sessionsTemp = (Dictionary<string, SessionToken>)formatter.Deserialize(fs);
        ...
        private static void SaveToDisk()
        {
            ...
            var sessionCacheLines = new List<string>();
            ...
            foreach (var entry in sessions)
                sessionCacheLines.Add(entry.Key + '=' + entry.Value);
